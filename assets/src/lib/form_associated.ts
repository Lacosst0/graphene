  ...
  "cds-multi-select": {
    ...
    parseEventValue: (event, el) => {
      ...
      if (Array.isArray(detail.selectedItems)) {
        const values = detail.selectedItems.map(
...
const importers: Record<string, Importer> = {
  ...
  "cds-time-picker": () =>
    import("@carbon/web-components/es/components/time-picker/time-picker.js"),
  "cds-slider": () =>
...
const parseMultiSelectValues = (el: HTMLElement): string[] => {
  const items = Array.from(
    el.querySelectorAll("cds-multi-select-item[selected]"),
  ) as any[];
  if (items.length > 0) {
    return items
      .map((item) => item?.value ?? item?.getAttribute?.("value") ?? "")
      .filter(Boolean);
  }
...
const createFormAssociatedClass = (
  Base: FormElementClass,
  spec: FormSpec,
): FormElementClass => {
  class FormAssociatedElement extends Base {
    ...
    private _shouldSync(
      changed: Map<string | number | symbol, unknown> | null | undefined,
    ): boolean {
      if (!changed) return true;
      if (spec.mode === "boolean") {
        const checkedProp = spec.checkedProp || "checked";
        return (
          changed.has(checkedProp) ||
          changed.has("checked") ||
          changed.has("toggled")
        );
      }
    ...
    private _bindEvents() {
      ...
      const handler: ChangeHandler = (event: Event) => {
        if (
          ...
          (event as any).__grapheneSynthetic
        ) {
          return;
        }
    ...
    private _syncFormValue() {
      if (
        !this._internals ||
        typeof this._internals.setFormValue !== "function"
      )
        return;
      let value: string;
...
export const ensureFormAssociatedTag = async (
  tagName: string,
): Promise<void> => {
