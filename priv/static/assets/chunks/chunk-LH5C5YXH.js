...
var CDSDropdown = class CDSDropdown2 extends ValidityMixin(HostListenerMixin(FormMixin(FocusMixin(i2)))) {
      ...
      switch (action) {
        case DROPDOWN_KEYBOARD_ACTION.NAVIGATING: {
          ...
          this.updateComplete.then(() => {
            ...
            const items = this.querySelectorAll(constructor2.selectorItem);
            if (items.length > 0) {
              const selectedItem2 = this.querySelector(constructor2.selectorItemSelected);
              const firstEnabledItem = Array.from(items).find((item) => !item.hasAttribute("disabled"));
              const initialItem = selectedItem2 && !selectedItem2.hasAttribute("disabled") ? selectedItem2 : firstEnabledItem !== null && firstEnabledItem !== void 0 ? firstEnabledItem : null;
              if (initialItem) {
                this._setHighlightedItem(initialItem, { scrollIntoView: true });
              } else {
                this._clearHighlight();
              }
              const menu = (_a2 = this.shadowRoot) === null || _a2 === void 0 ? void 0 : _a2.getElementById("menu-body");
  ...
  _handleMenuInputKeydown(event) {
    if (event.defaultPrevented || !this._supportsMenuInputFiltering || !this._menuInputNode) {
      return false;
    }
  ...
  _forwardKeyToMenuInput(event, input) {
    ...
    if (key === "Backspace") {
      ...
      if (selectionStart === input.value.length && selectionStart === selectionEnd) {
        return;
      }
  ...
  _handleMouseleaveInner(event) {
    ...
    if (this._supportsMenuInputFiltering && this._menuInputNode && ((_c = this.shadowRoot) === null || _c === void 0 ? void 0 : _c.activeElement) === this._menuInputNode) {
      return;
    }
    this._clearHighlight();
  }
  ...
  _handleAILabelSlotChange({ target }) {
    var _a;
    const decoratorNodes = target.assignedNodes().filter((elem) => elem.matches !== void 0 ? elem.matches(this.constructor.aiLabelItem) || // remove reference to slug in v12
    ...
    if (!disabled) {
      if (this.dispatchEvent(new CustomEvent(eventBeforeToggle, init))) {
        ...
        if (this.open) {
          ...
          this.updateComplete.then(() => {
            var _a2;
            const trigger = (_a2 = this.shadowRoot) === null || _a2 === void 0 ? void 0 : _a2.getElementById("trigger-button");
            trigger === null || trigger === void 0 ? void 0 : trigger.focus();
          });
        }
        if (!this.open) {
  ...
  _getDropdownItemFromEvent(event) {
    ...
    for (const node of path) {
      if (node instanceof Element && typeof node.matches === "function" && node.matches(selector)) {
        return node;
      }
    }
    return null;
  ...
  _navigate(direction) {
    ...
    while (nextIndex >= 0 && nextIndex < items.length && ((_a = items[nextIndex]) === null || _a === void 0 ? void 0 : _a.hasAttribute("disabled"))) {
      nextIndex += direction;
    }
  ...
  updated(changedProperties) {
    ...
    if (this.autoalign && this.open && (changedProperties.has("open") || changedProperties.has("autoalign") && this.autoalign || changedProperties.has("direction") || changedProperties.has("size"))) {
  ...
  _updateAutoAlignPlacement() {
    ...
    const alignment = this.direction === DROPDOWN_DIRECTION.TOP ? DROPDOWN_DIRECTION.TOP : DROPDOWN_DIRECTION.BOTTOM;
  ...
  render() {
    ...
    const helperClasses = e2({
      [`${prefix}--form__helper-text`]: true,
      [`${prefix}--form__helper-text--disabled`]: normalizedProps.disabled
    });
    const iconContainerClasses = e2({
    ...
    const validityIcon = !normalizedProps.invalid ? void 0 : iconLoader(_16, {
      class: `${prefix}--list-box__invalid-icon`,
      "aria-label": toggleLabel
    });
    const warningIcon = !normalizedProps.warn ? void 0 : iconLoader(_162, {
    ...
    const menuBody = b`
      <div
        aria-labelledby="${o(ariaLabel ? void 0 : "dropdown-label")}"
        aria-label="${o(ariaLabel ? ariaLabel : void 0)}"
        id="menu-body"
        part="menu-body"
    ...
    return b`
      ...
      <div
        ...
        <div
          id="${o(!shouldTriggerBeFocusable ? void 0 : "trigger-button")}"
          class="${prefix}--list-box__field"
          part="trigger-button"
          tabindex="${o(!shouldTriggerBeFocusable ? void 0 : "0")}"
          role="${o(!shouldTriggerBeFocusable ? void 0 : "combobox")}"
          aria-label="${o(ariaLabel ? ariaLabel : void 0)}"
          aria-labelledby="${o(!shouldTriggerBeFocusable ? void 0 : "dropdown-label")}"
          aria-expanded="${o(!shouldTriggerBeFocusable ? void 0 : String(open))}"
          aria-haspopup="${o(!shouldTriggerBeFocusable ? void 0 : "listbox")}"
          aria-controls="${o(!shouldTriggerBeFocusable ? void 0 : "menu-body")}"
          aria-activedescendant="${o(!shouldTriggerBeFocusable ? void 0 : open ? activeDescendant !== null && activeDescendant !== void 0 ? activeDescendant : activeDescendantFallback : "")}">
          ${this._renderPrecedingLabel()}${this._renderLabel()}${this._renderFollowingLabel()}
          <div id="trigger-caret" class="${iconContainerClasses}">
            ${iconLoader(_163, { "aria-label": toggleLabel })}
          </div>
        </div>
...
CDSDropdown.TRIGGER_KEYS = /* @__PURE__ */ new Set([" ", "Enter"]);
CDSDropdown.shadowRootOptions = Object.assign(Object.assign({}, i2.shadowRootOptions), { delegatesFocus: true });
...
__decorate([
  e("#menu-body")
], CDSDropdown.prototype, "_menuBodyNode", void 0);
__decorate([
  e("#trigger-button")
], CDSDropdown.prototype, "_triggerButtonNode", void 0);
__decorate([
  e('slot[name="helper-text"]')
], CDSDropdown.prototype, "_slotHelperTextNode", void 0);
__decorate([
  e('slot[name="title-text"]')
...
__decorate([
  n({ attribute: "toggle-label-closed" })
], CDSDropdown.prototype, "toggleLabelClosed", void 0);
__decorate([
  n({ attribute: "toggle-label-open" })
