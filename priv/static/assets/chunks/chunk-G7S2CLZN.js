...
var ne = (i3, S) => {
  ...
  const l = {}, h = 4, { onViewChangeStart: v, onViewChangeEnd: k, excludeSwipeSupport: E, useMaxHeight: $ } = S || {}, V = (e, t2) => {
    ...
    return { currentIndex: n[0], lastIndex: parseInt(t2?.dataset.index || n[0].toString(), 10), totalViews: e, historyStack: o3 };
  }, H = () => {
    m = [...n];
    const e = L();
    v?.(e);
...
function i(a, o3) {
  const t2 = new Intl.RelativeTimeFormat(o3?.locale, { style: o3?.style ?? "long" }), f = typeof a == "number" ? new Date(a) : a, m = Date.now(), l = Math.floor((m - f.getTime()) / 1e3), r3 = Math.floor(l / 60), n = Math.floor(r3 / 60), e = Math.floor(n / 24), s = Math.floor(e / 7), c = Math.floor(s / 4), h = Math.floor(e / 365);
  return Math.abs(l) < 60 ? new Intl.RelativeTimeFormat(o3?.locale, { numeric: "auto", style: o3?.style ?? "long" }).format(0, "seconds") : Math.abs(r3) < 60 ? t2.format(r3 * -1, "minutes") : Math.abs(n) < 24 ? t2.format(n * -1, "hours") : Math.abs(e) < 7 ? t2.format(e * -1, "days") : Math.abs(s) < 4 ? t2.format(s * -1, "weeks") : Math.abs(e) < 365 ? t2.format(c * -1, "months") : t2.format(h * -1, "years");
}
...
function i2(l, t2) {
  return new Intl.DateTimeFormat(t2?.locale, { timeStyle: t2?.style ?? "short", timeZone: t2?.timeZone }).format(l);
}
function r(l, t2) {
  return new Intl.DateTimeFormat(t2?.locale, { dateStyle: t2?.style ?? "medium", timeZone: t2?.timeZone }).format(l);
}
function o(l, t2) {
  const e = t2?.timeStyle ?? (t2?.style === "tooltip" ? "long" : t2?.style) ?? "short", m = t2?.dateStyle ?? (t2?.style === "tooltip" ? "full" : t2?.style) ?? "medium";
  return new Intl.DateTimeFormat(t2?.locale, { timeStyle: e, dateStyle: m, timeZone: t2?.timeZone }).format(l);
}
function y(l, t2, e) {
  const m = e?.timeStyle === null ? void 0 : e?.timeStyle ?? e?.style ?? "short", a = e?.dateStyle === null ? void 0 : e?.dateStyle ?? e?.style ?? "medium";
  return new Intl.DateTimeFormat(e?.locale, { timeStyle: m, dateStyle: a, timeZone: e?.timeZone }).formatRange(l, t2);
}
